  ...
  <div class="fixed inset-0 z-[9999] flex items-center justify-center px-4 py-6 pointer-events-none">
    <transition-group name="notification" tag="div" class="flex w-full max-w-sm flex-col gap-3">
      <div
        ...
        :key="notification.id"
        :class="[
          'pointer-events-auto w-full overflow-hidden rounded-[28px] border border-slate-200/80 bg-white/95 shadow-2xl backdrop-blur-xl ring-1 ring-slate-900/5 transition duration-300 ease-out',
          notificationVariants(notification.type),
        ]"
      >
        <div class="relative p-5">
          <div class="flex items-start gap-4">
            <div class="mt-1 flex h-11 w-11 items-center justify-center rounded-2xl bg-slate-100 text-slate-900">
            ...
            <div class="flex-1">
              <p class="text-sm font-semibold uppercase tracking-[0.12em] text-slate-900">
            ...
            <button
              ...
              class="ml-2 rounded-full p-2 text-slate-500 transition hover:bg-slate-100 hover:text-slate-900"
            >
...
const notificationVariants = (type) => {
  switch (type) {
    case 'success':
      return 'border-emerald-200/80 bg-emerald-50/80 text-slate-900';
    case 'error':
      return 'border-rose-200/80 bg-rose-50/80 text-slate-900';
    case 'warning':
      return 'border-amber-200/80 bg-amber-50/80 text-slate-900';
    default:
      return 'border-sky-200/80 bg-sky-50/80 text-slate-900';
  }
};
