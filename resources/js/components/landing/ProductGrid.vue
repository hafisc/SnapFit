  ...
  <section id="marketplace" class="pt-10 pb-24 relative overflow-hidden bg-transparent">
    ...
    <div class="max-w-[1520px] mx-auto px-6 relative z-10">
        ...
        <div class="overflow-hidden rounded-3xl relative">
          ...
          <div class="flex transition-transform duration-700"
            ...
            <div v-for="(slide, index) in promoSlides" :key="index"
              class="min-w-full h-[160px] md:h-[200px] lg:h-[220px] relative rounded-[24px] overflow-hidden bg-gradient-to-br from-[#5C4033] via-[#8B6F47] to-[#A0845C]">
              <!-- Background -->
              <img :src="slide.image" class="absolute inset-0 w-full h-full object-cover"
                @error="$event.target.style.display = 'none'" />

              <!-- Multi-layer depth overlays -->
              <div class="absolute inset-0 bg-gradient-to-r from-black/60 via-black/35 to-black/10"></div>
              <div class="absolute inset-0 bg-gradient-to-t from-black/30 via-transparent to-black/10"></div>
              <!-- Subtle radial vignette -->
              <div class="absolute inset-0"
                style="background: radial-gradient(ellipse at 75% 50%, transparent 40%, rgba(0,0,0,0.3) 100%);"></div>
              <!-- Decorative batik pattern overlay -->
              <div class="absolute inset-0 opacity-[0.08] mix-blend-overlay"
                style="background-image: url('/assets/batik-bg.png'); background-size: 280px; background-repeat: repeat;">
              </div>
              ...
              <div class="relative z-10 h-full flex items-center">
                <div class="w-full max-w-[1520px] mx-auto px-6 md:px-10">
                  ...
                  <div class="max-w-2xl text-white">
                    <span v-if="slide.tag"
                      class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/30 backdrop-blur-sm border border-white/20 text-[10px] font-black uppercase tracking-[0.2em] mb-3">
                      <span class="w-1.5 h-1.5 bg-orange-400 rounded-full"></span>
                      {{ slide.tag }}
                    </span>

                    <h3 class="text-xl md:text-2xl lg:text-3xl font-black mb-2 leading-tight tracking-tight">
                      {{ slide.title }}
                    </h3>

                    <p class="text-xs md:text-sm text-white/85 mb-4 max-w-lg leading-relaxed">
                      {{ slide.description }}
                    </p>

                    <button
                      class="bg-terracotta px-5 py-2.5 rounded-xl font-bold text-[11px] uppercase tracking-widest hover:bg-terracottaDark transition-all shadow-lg hover:shadow-terracotta/30">
                      Jelajahi Koleksi
                    </button>
                  </div>
      ...
      <div v-if="isHomepage" class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-9" data-aos="fade-up" data-aos-delay="50">
        ...
        <!-- AI Studio -->
        <div
          class="bg-surface border border-borderSoft min-h-[82px] p-4 rounded-2xl shadow-sm hover:shadow-md hover:border-terracotta transition-all duration-300 flex items-center gap-4 cursor-pointer">
        ...
        <!-- AR Try-On -->
        <div
          class="bg-surface border border-borderSoft min-h-[82px] p-4 rounded-2xl shadow-sm hover:shadow-md hover:border-indigo transition-all duration-300 flex items-center gap-4 cursor-pointer">
        ...
        <!-- Co-Create Room -->
        <div
          class="bg-surface border border-borderSoft min-h-[82px] p-4 rounded-2xl shadow-sm hover:shadow-md hover:border-olive transition-all duration-300 flex items-center gap-4 cursor-pointer">
      ...
      <div class="flex items-center justify-between mb-4 mt-6">
        <h2 class="text-lg md:text-xl font-black text-espresso flex items-center gap-2.5">
          <span class="w-1.5 h-5 bg-terracotta rounded-full"></span>
          Koleksi Pilihan Nusantara
        </h2>
        <a href="#"
          class="lihat-semua-link text-sm font-extrabold text-terracotta hover:text-terracottaDark transition-all flex items-center gap-1.5 group/link">
          Lihat Semua
          <svg class="w-4 h-4 transition-transform group-hover/link:translate-x-0.5" fill="none" stroke="currentColor"
      ...
      <div class="flex flex-wrap gap-1.5 mb-5">
        <button v-for="cat in filterChips" :key="cat.value" @click="activeCategory = cat.value"
          class="px-3 py-1.5 rounded-full text-[11px] font-bold transition-all duration-200 border" :class="activeCategory === cat.value
            ? 'bg-terracotta text-white border-terracotta shadow-sm'
            : 'bg-surface text-muted border-borderSoft hover:border-terracotta/40 hover:text-espresso'">
          {{ cat.label }}
        </button>
      </div>
        ...
        <div v-for="(product, index) in filteredProducts" :key="product.id" @click="viewProductDetail(product)"
          ...
          <div class="relative w-full h-[170px] bg-gray-100 isolate overflow-hidden border-b border-borderSoft">
            ...
            <div class="absolute top-2 left-2 z-20">
              <span
                class="inline-flex items-center gap-1.5 bg-surface/95 backdrop-blur-md px-2 py-1 rounded-full text-[8px] font-black uppercase tracking-widest shadow-sm border border-terracotta/20">
            ...
            <div
              ...
              <button @click.stop="toggleWishlist(product)"
                class="w-8 h-8 bg-surface/95 backdrop-blur-md rounded-full flex items-center justify-center hover:bg-surface hover:scale-110 active:scale-95 transition-all shadow-sm"
                :class="isWishlisted(product.id) ? 'text-red-500' : 'text-espresso'">
            ...
            <div
              ...
              <button @click.stop="addToCart(product)"
                class="w-full bg-terracotta text-white py-2 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-terracottaDark shadow-lg active:scale-95 transition-all flex items-center justify-center gap-1.5">
          ...
          <!-- Product Info -->
          <div class="p-2.5 pb-2 flex flex-col flex-1">
            <h3
              class="font-bold text-[13px] mb-0.5 text-espresso line-clamp-2 leading-snug group-hover:text-terracotta transition-colors">
              {{ product.name }}
            </h3>

            <div class="flex items-center gap-1.5 text-[10px] text-muted mb-0.5">
              <span v-if="product.origin" class="flex items-center gap-0.5">
            ...
            <p v-if="product.umkm_name" class="text-[10px] text-muted mb-1 leading-tight">
              by <span class="font-semibold text-espresso/70">{{ product.umkm_name }}</span>
            </p>

            <div class="flex items-center gap-1 mb-1">
              <svg class="w-2.5 h-2.5 text-gold fill-current" viewBox="0 0 20 20">
                <path
              ...
              <span class="text-[10px] text-muted">• {{ product.sold || '0' }} terjual</span>
            </div>

            <!-- Spacer -->
            <div class="flex-1"></div>

            <div v-if="product.badges && product.badges.length" class="flex flex-wrap gap-0.5 mb-1">
              <span v-for="badge in product.badges.slice(0, 2)" :key="badge"
                class="text-[7px] px-1.5 py-px border border-terracotta/25 text-terracotta rounded bg-terracotta/5 font-semibold">
                {{ badge }}
              </span>
            </div>
            ...
            <div class="flex items-center justify-between mt-auto">
              <p class="text-[15px] font-bold text-terracotta tracking-tight">
                <span class="text-[10px] font-semibold mr-0.5">Rp</span>{{ Number(product.price).toLocaleString('id-ID')
                }}
              </p>
            </div>
          </div>
...
/* Lihat Semua Hover Underline */
.lihat-semua-link {
  position: relative;
  text-decoration: none;
}

.lihat-semua-link::after {
  content: '';
  position: absolute;
  left: 0;
  bottom: -2px;
