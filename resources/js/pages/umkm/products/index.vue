<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      <div v-for="i in 4" :key="i" class="bg-white rounded-3xl p-4 border border-slate-100 animate-pulse">
      ...
      <div v-for="product in products" :key="product.id" class="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] overflow-hidden group hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all">
        <div class="relative h-56 w-full bg-slate-100 overflow-hidden">
          ...
          <div class="absolute top-4 right-4 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <router-link :to="`/umkm/products/edit/${product.id}`" class="w-8 h-8 rounded-lg bg-white/90 text-slate-700 flex items-center justify-center shadow-lg hover:bg-white transition-colors" title="Edit">
