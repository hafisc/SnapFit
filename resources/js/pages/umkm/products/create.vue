<template>
  <div class="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-xl font-black text-slate-800 tracking-tight">Tambah Produk Baru</h2>
    ...
    <form @submit.prevent="submit" class="bg-white p-8 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
          <div>
            ...
            <input v-model="form.name" type="text" required class="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all" placeholder="Contoh: Kemeja Batik Cap Premium" />
          </div>
          ...
          <div>
            ...
            <input v-model="form.price" type="number" min="0" required class="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all" placeholder="150000" />
          </div>
          
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">Stok Tersedia</label>
            <input v-model="form.stock" type="number" min="0" required class="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all" placeholder="50" />
          </div>
        </div>
        ...
        <div class="space-y-6">
          <div>
            ...
            <textarea v-model="form.description" required rows="5" class="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-medium text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all resize-none" placeholder="Jelaskan detail material, ukuran, dan keunggulan produk..."></textarea>
          </div>
          
          <div>
            <label class="block text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2 ml-1">URL Gambar Produk</label>
            <input v-model="form.image_url" type="url" class="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all" placeholder="https://unsplash.com/foto.jpg" />
            <p class="text-[10px] text-slate-400 mt-2 font-medium">Dalam versi demo, gunakan URL gambar (misal dari Unsplash).</p>
          </div>
      ...
      <div class="pt-6 border-t border-slate-100 flex items-center justify-between">
        <label class="flex items-center gap-3 cursor-pointer group">
          ...
          <span class="text-xs font-black uppercase tracking-widest" :class="form.is_published ? 'text-orange-600' : 'text-slate-400'">Langsung Publish</span>
        </label>
