  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-1 space-y-6">
        <form @submit.prevent="generateAI" class="bg-surface rounded-2xl border border-borderSoft p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] space-y-6">
          
          <!-- Image Upload Area -->
          <div>
            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2 ml-1">Foto Asli Produk</label>
            <div class="border-2 border-dashed border-borderSoft rounded-2xl p-6 text-center hover:border-terracotta hover:bg-sand transition-colors cursor-pointer" @click="$refs.fileInput.click()">
              ...
              <div v-else class="space-y-2 py-4">
                <div class="w-12 h-12 bg-sand text-terracotta rounded-full flex items-center justify-center mx-auto">
                  <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                </div>
                <p class="text-sm font-bold text-muted">Klik untuk unggah foto</p>
            ...
            <div class="mt-4">
               ...
               <input v-model="form.original_image_url" type="url" placeholder="https://contoh.com/foto.jpg" class="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-sm font-bold text-espresso outline-none focus:ring-2 focus:ring-orange-200 transition-all" />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea v-model="form.prompt" rows="3" placeholder="Contoh: Tolong buatkan konsep foto yang elegan untuk tas kulit ini agar cocok diposting di Instagram saat Hari Raya..." class="w-full bg-slate-50 border-none rounded-xl px-4 py-3 text-sm font-medium text-espresso outline-none focus:ring-2 focus:ring-orange-200 transition-all resize-none"></textarea>
            
            <!-- Quick Prompts -->
            <div class="flex flex-wrap gap-2 mt-3">
              <button type="button" @click="form.prompt = 'Tolong berikan arahan gaya foto minimalis dan elegan'" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-muted rounded-lg text-[10px] font-bold transition-colors">Minimalis</button>
              <button type="button" @click="form.prompt = 'Tolong buatkan konsep foto yang ceria, terang, dan menonjolkan warna produk'" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-muted rounded-lg text-[10px] font-bold transition-colors">Vibrant</button>
              <button type="button" @click="form.prompt = 'Tolong berikan referensi gaya foto aesthetic hangat (warm tone)'" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-muted rounded-lg text-[10px] font-bold transition-colors">Aesthetic Hangat</button>
            </div>
          </div>
          ...
          <button type="submit" :disabled="isGenerating || (!selectedFile && !form.original_image_url)" class="w-full bg-slate-900 hover:bg-black text-white px-6 py-4 rounded-xl text-xs font-bold tracking-wider uppercase transition-all shadow-lg shadow-slate-900/20 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2">
            <svg v-if="isGenerating" class="animate-spin w-4 h-4 text-terracotta" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
      ...
      <div class="lg:col-span-2">
        <div class="bg-surface rounded-2xl border border-borderSoft p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] min-h-full">
          <div class="flex items-center justify-between mb-6">
            <h3 class="text-sm font-bold text-espresso uppercase tracking-wider">Riwayat Analisis AI</h3>
            <button @click="fetchHistory(1)" class="text-slate-400 hover:text-terracotta transition-colors">
          ...
          <div v-if="loadingHistory" class="flex items-center justify-center py-20 text-slate-400">
            <svg class="animate-spin w-8 h-8 text-terracotta" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          </div>
          ...
          <div v-else-if="history.length === 0" class="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p class="text-muted font-bold mb-1">Belum Ada Analisis AI</p>
          ...
          <div v-else class="space-y-12">
            <div v-for="item in history" :key="item.id" class="border-b border-borderSoft pb-12 last:border-0 relative group">
              ...
              <div class="flex flex-col sm:flex-row items-start gap-6 mb-6">
                <!-- Original Image -->
                <div class="w-full sm:w-48 shrink-0 relative rounded-xl overflow-hidden border border-borderSoft bg-slate-50">
                ...
                <div class="flex-1 w-full min-w-0">
                  <div class="flex items-center gap-3 mb-2">
                    <span class="bg-sand text-orange-700 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider">Report</span>
                    <span class="text-[10px] text-slate-400 uppercase tracking-wider font-bold">{{ formatDate(item.created_at) }}</span>
                  </div>
                  
                  <p v-if="item.prompt" class="text-sm font-bold text-espresso italic mb-4 bg-slate-50 p-3 rounded-lg border border-borderSoft border-l-4 border-l-orange-400">
                    "{{ item.prompt }}"
                  </p>
                  
                  <!-- Markdown Analysis -->
                  <div class="prose prose-sm max-w-none prose-orange prose-headings:font-bold prose-a:text-terracotta prose-img:rounded-xl">
                  ...
                  <div v-if="item.generated_images && item.generated_images.length > 0" class="mt-8">
                     ...
                     <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div v-for="(img, idx) in item.generated_images" :key="idx" class="relative aspect-square rounded-xl overflow-hidden group/img cursor-pointer border border-borderSoft">
            ...
            <div v-if="pagination.last_page > 1" class="flex justify-center pt-4">
              <button v-if="pagination.current_page < pagination.last_page" @click="fetchHistory(pagination.current_page + 1)" class="text-xs font-bold text-terracotta hover:text-orange-700 uppercase tracking-wider">
                Muat Lebih Banyak...
              </button>
            </div>
          </div>
