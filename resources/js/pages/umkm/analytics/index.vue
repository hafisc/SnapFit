<template>
  <div class="space-y-5 max-w-[1400px]">
    ...
    <div class="flex items-center justify-between">
      <div class="flex bg-[#F8F1E7] rounded-xl p-1">
        <button v-for="p in periods" :key="p" @click="activePeriod = p"
          class="px-3.5 py-2 text-[11px] font-bold rounded-lg transition-all"
          :class="activePeriod === p ? 'bg-white text-[#2B1E16] shadow-sm' : 'text-[#8A7A6C] hover:text-[#2B1E16]'">{{ p }}</button>
      </div>
      <button class="px-4 py-2 bg-white border border-[#E8DCCB] rounded-xl text-[11px] font-bold text-[#2B1E16] hover:bg-[#F8F1E7] transition-colors flex items-center gap-1.5">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div v-for="k in kpis" :key="k.label" class="bg-white rounded-2xl border border-[#E8DCCB]/60 p-5">
        ...
        <div class="flex items-center gap-1 mt-1">
          <span class="text-[10px] font-bold" :class="k.up ? 'text-[#0F8A4B]' : 'text-[#D94A38]'">{{ k.up ? '↑' : '↓' }} {{ k.change }}</span>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-white rounded-2xl border border-[#E8DCCB]/60 p-6">
        ...
        <p class="text-[11px] text-[#8A7A6C] font-medium mb-6">6 bulan terakhir</p>
        <div class="h-44 flex items-end gap-2">
          <div v-for="(m, i) in revenueData" :key="i" class="flex-1 rounded-t-lg transition-all duration-500 cursor-pointer group relative"
            :class="i === revenueData.length - 1 ? 'bg-gradient-to-t from-[#B85C38] to-[#D4845A]' : 'bg-gradient-to-t from-[#E8DCCB] to-[#F0E6D8] hover:from-[#D4C4B0] hover:to-[#E8DCCB]'" :style="{ height: m.pct + '%' }">
            <div class="absolute -top-8 left-1/2 -translate-x-1/2 bg-[#2B1E16] text-white text-[10px] font-bold px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap z-10">Rp {{ m.value }}M</div>
          </div>
        </div>
        <div class="flex justify-between mt-3 text-[10px] text-[#8A7A6C] font-medium">
      ...
      <div class="bg-white rounded-2xl border border-[#E8DCCB]/60 p-6">
        ...
        <p class="text-[11px] text-[#8A7A6C] font-medium mb-6">Views vs Konversi per produk</p>
        <div class="space-y-4">
          <div v-for="p in productPerf" :key="p.name">
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-[12px] font-semibold text-[#2B1E16] truncate">{{ p.name }}</span>
              <span class="text-[11px] font-bold text-[#2B1E16]">{{ p.conversion }}%</span>
            </div>
            ...
            <div class="flex items-center justify-between mt-1">
              <span class="text-[10px] text-[#8A7A6C]">{{ p.views }} views</span>
              <span class="text-[10px] text-[#8A7A6C]">{{ p.orders }} orders</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <div class="bg-white rounded-2xl border border-[#E8DCCB]/60 p-6">
        ...
        <div class="space-y-3">
          <div v-for="d in demographics" :key="d.city" class="flex items-center justify-between py-2 border-b border-[#E8DCCB]/40 last:border-0">
            <div class="flex items-center gap-2.5">
              <span class="w-2 h-2 rounded-full" :class="d.dotClass"></span>
              <span class="text-[12px] font-medium text-[#2B1E16]">{{ d.city }}</span>
            </div>
            <div class="text-right">
              <span class="text-[12px] font-bold text-[#2B1E16]">{{ d.orders }}</span>
              <span class="text-[10px] text-[#8A7A6C] ml-1">pesanan</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- AI Studio Stats -->
      <div class="bg-white rounded-2xl border border-[#E8DCCB]/60 p-6">
        <h3 class="text-[14px] font-bold text-[#2B1E16] mb-5">AI Studio Usage</h3>
        <div class="space-y-4">
          <div v-for="a in aiStats" :key="a.label">
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-[12px] font-medium text-[#6F6259]">{{ a.label }}</span>
        ...
        <div class="mt-5 p-3 rounded-xl bg-[#B85C38]/5 border border-[#B85C38]/15">
          <p class="text-[11px] text-[#B85C38] font-medium">💡 Estimasi biaya dihemat bulan ini: <span class="font-bold">Rp 2.500.000</span></p>
        </div>
      </div>
    </div>
  </div>
