  ...
  <div class="space-y-5 max-w-[1400px]">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      <div v-for="k in kpis" :key="k.label" class="bg-white rounded-2xl border border-slate-200/60 p-5">
        ...
        <p class="text-[22px] font-bold text-slate-800 leading-none mb-1">{{ k.value }}</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-white rounded-2xl border border-slate-200/60 p-6">
        <h3 class="text-[14px] font-bold text-slate-800 mb-1">Tren Pendapatan</h3>
        ...
        <div class="h-44 flex items-end gap-2">
          <div v-for="(m, i) in revenueData" :key="i" class="flex-1 rounded-t-md transition-all duration-500 cursor-pointer group relative"
            :class="i === revenueData.length - 1 ? 'bg-orange-500' : 'bg-orange-200 hover:bg-orange-300'" :style="{ height: m.pct + '%' }">
      ...
      <!-- Product Performance -->
      <div class="bg-white rounded-2xl border border-slate-200/60 p-6">
        <h3 class="text-[14px] font-bold text-slate-800 mb-1">Performa Produk</h3>
        ...
        <div class="space-y-4">
          <div v-for="p in productPerf" :key="p.name">
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-[12px] font-semibold text-slate-700 truncate">{{ p.name }}</span>
              <span class="text-[11px] font-bold text-slate-800">{{ p.conversion }}%</span>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <!-- Traffic Sources -->
      <div class="bg-white rounded-2xl border border-slate-200/60 p-6">
        <h3 class="text-[14px] font-bold text-slate-800 mb-5">Sumber Traffic</h3>
        <div class="space-y-3">
          <div v-for="t in trafficSources" :key="t.name" class="flex items-center gap-3">
            ...
            <div class="flex-1">
              <div class="flex items-center justify-between mb-1">
                <span class="text-[12px] font-semibold text-slate-700">{{ t.name }}</span>
                <span class="text-[11px] font-bold text-slate-800">{{ t.pct }}%</span>
              </div>
              <div class="h-1.5 bg-slate-100 rounded-full overflow-hidden">
      ...
      <div class="bg-white rounded-2xl border border-slate-200/60 p-6">
        <h3 class="text-[14px] font-bold text-slate-800 mb-5">Demografi Pembeli</h3>
        <div class="space-y-3">
          <div v-for="d in demographics" :key="d.city" class="flex items-center justify-between py-2 border-b border-slate-50 last:border-0">
            <div class="flex items-center gap-2.5">
              ...
              <span class="text-[12px] font-medium text-slate-700">{{ d.city }}</span>
            </div>
            <div class="text-right">
              <span class="text-[12px] font-bold text-slate-800">{{ d.orders }}</span>
      ...
      <!-- AI Studio Stats -->
      <div class="bg-white rounded-2xl border border-slate-200/60 p-6">
        <h3 class="text-[14px] font-bold text-slate-800 mb-5">AI Studio Usage</h3>
        <div class="space-y-4">
          <div v-for="a in aiStats" :key="a.label">
            <div class="flex items-center justify-between mb-1.5">
              <span class="text-[12px] font-medium text-slate-600">{{ a.label }}</span>
        ...
        <div class="mt-5 p-3 rounded-xl bg-orange-50 border border-orange-100">
...
const trafficSources = [
  { name: 'SnapFit Marketplace', pct: 52, emoji: '🏪', bgClass: 'bg-orange-50', barClass: 'bg-orange-400' },
...
const demographics = [
  { city: 'Kota Malang', orders: 42, dotClass: 'bg-orange-500' },
...
const aiStats = [
  { label: 'Studio Photo Generated', value: '12', pct: 60, valueClass: 'text-orange-600', barClass: 'bg-orange-400' },
