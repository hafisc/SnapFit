<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
    <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-8 text-white relative overflow-hidden shadow-xl shadow-orange-500/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
      ...
      <button @click="showCreateModal = true" class="relative z-10 bg-white text-orange-600 hover:bg-orange-50 px-6 py-4 rounded-xl text-xs font-bold tracking-wider uppercase transition-all shadow-lg flex items-center gap-2 flex-shrink-0">
      ...
      <div v-for="room in rooms" :key="room.id" class="bg-white rounded-2xl border border-slate-100 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all flex flex-col justify-between group">
        <div>
          <div class="flex items-start justify-between mb-4">
            <div class="w-12 h-12 bg-orange-50 text-orange-500 rounded-2xl flex items-center justify-center text-xl group-hover:scale-110 transition-transform">
        ...
        <div class="border-t border-slate-50 pt-4 flex items-center justify-between mt-auto">
          <div class="flex -space-x-2">
            <div v-for="i in Math.min(room.participants_count || 1, 3)" :key="i" class="w-8 h-8 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-600 z-10 relative">
               {{ i === 1 ? room.creator?.name?.charAt(0) : 'D' }}
            </div>
            <div v-if="(room.participants_count || 1) > 3" class="w-8 h-8 rounded-full border-2 border-white bg-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-500 z-0 relative">
              +{{ (room.participants_count || 1) - 3 }}
            </div>
          </div>
          <button @click="joinRoom(room.id)" class="text-xs font-bold uppercase tracking-wider text-orange-600 hover:text-orange-800 transition-colors">
            Gabung &rarr;
          </button>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full relative z-10 animate-in zoom-in-95 duration-200">
        ...
        <form @submit.prevent="createRoom" class="space-y-4">
          <div>
            ...
            <input v-model="form.name" type="text" required class="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-bold text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all" placeholder="Contoh: Desain Batik Summer 2026" />
          </div>
          <div>
            <label class="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2 ml-1">Deskripsi & Tujuan</label>
            <textarea v-model="form.description" rows="3" required class="w-full bg-slate-50 border-none rounded-xl px-5 py-4 text-sm font-medium text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all resize-none" placeholder="Mencari desainer untuk membuat pola baru..."></textarea>
          </div>
          <div class="flex gap-4 pt-4">
            ...
            <button type="submit" :disabled="isCreating" class="flex-1 bg-orange-500 hover:bg-orange-600 text-white py-4 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50">
