<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
    <div class="bg-gradient-to-br from-[#2B1E16] via-[#3D2B1F] to-[#4A3428] rounded-2xl p-8 text-white relative overflow-hidden shadow-lg flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
      ...
      <button @click="showCreateModal = true" class="relative z-10 bg-white text-[#B85C38] hover:bg-[#F8F1E7] px-6 py-4 rounded-xl text-xs font-bold tracking-wider uppercase transition-all shadow-lg flex items-center gap-2 flex-shrink-0">
    ...
    <div v-if="loading" class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      <div v-for="i in 3" :key="i" class="bg-white rounded-3xl p-6 border border-[#E8DCCB] animate-pulse h-48"></div>
    </div>
    
    <div v-else-if="rooms.length === 0" class="bg-white rounded-2xl border border-[#E8DCCB] shadow-[0_8px_30px_rgb(0,0,0,0.02)] py-20 text-center">
      <div class="w-20 h-20 bg-[#F8F1E7] text-orange-400 rounded-full flex items-center justify-center mx-auto mb-4 text-3xl">🪑</div>
      <h3 class="text-lg font-bold text-[#2B1E16] mb-1">Belum Ada Ruang Aktif</h3>
      <p class="text-[#8A7A6C] text-sm font-medium mb-6">Mulai kolaborasi pertamamu dengan membuat ruangan baru.</p>
    </div>

    <div v-else class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
      <div v-for="room in rooms" :key="room.id" class="bg-white rounded-2xl border border-[#E8DCCB] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all flex flex-col justify-between group">
        <div>
        ...
        <div class="border-t border-[#E8DCCB]/40 pt-4 flex items-center justify-between mt-auto">
          <div class="flex -space-x-2">
            <div v-for="i in Math.min(room.participants_count || 1, 3)" :key="i" class="w-8 h-8 rounded-full border-2 border-white bg-[#E8DCCB] flex items-center justify-center text-xs font-bold text-[#6F6259] z-10 relative">
               {{ i === 1 ? room.creator?.name?.charAt(0) : 'D' }}
            </div>
            <div v-if="(room.participants_count || 1) > 3" class="w-8 h-8 rounded-full border-2 border-white bg-[#F8F1E7] flex items-center justify-center text-[10px] font-bold text-[#6F6259] z-0 relative">
              +{{ (room.participants_count || 1) - 3 }}
            </div>
          </div>
          <button @click="joinRoom(room.id)" class="text-xs font-bold uppercase tracking-wider text-[#B85C38] hover:text-orange-800 transition-colors">
            Gabung &rarr;
          </button>
        </div>
      </div>
      ...
      <div class="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full relative z-10 animate-in zoom-in-95 duration-200">
        ...
        <form @submit.prevent="createRoom" class="space-y-4">
          ...
          <div class="flex gap-4 pt-4">
            ...
            <button type="submit" :disabled="isCreating" class="flex-1 bg-[#B85C38] hover:bg-[#B85C38] text-white py-4 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors disabled:opacity-50">
