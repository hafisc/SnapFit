    ...
    <div class="flex-1 bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col overflow-hidden relative">
      ...
      <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-white z-10">
        ...
        <div class="flex items-center gap-3">
          <div class="flex -space-x-2 mr-2">
            <div v-for="p in room?.participants || []" :key="p.id" class="w-8 h-8 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-xs font-bold text-slate-600 shadow-sm" :title="p.name">
      ...
      <div ref="containerEl" class="flex-1 bg-slate-50 relative overflow-hidden group">
        ...
        <div class="absolute inset-0 flex flex-col items-center justify-center text-center p-8 pointer-events-none transition-opacity duration-500" :class="hasContent ? 'opacity-0' : 'opacity-100'">
          <div class="w-20 h-20 bg-white rounded-3xl shadow-lg flex items-center justify-center text-3xl mb-4 -rotate-6 group-hover:rotate-0 transition-transform duration-500">
            ✨
          </div>
        ...
        <!-- Floating Toolbar -->
        <div class="absolute bottom-6 left-1/2 -translate-x-1/2 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 flex items-center gap-2 z-20">
          <button @click="setMode('draw')" :class="mode === 'draw' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-orange-50 hover:text-orange-600'" class="w-10 h-10 rounded-xl flex items-center justify-center transition-colors" title="Menggambar (Pen)">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
          </button>
          <button @click="setMode('select')" :class="mode === 'select' ? 'bg-orange-50 text-orange-600' : 'text-slate-600 hover:bg-orange-50 hover:text-orange-600'" class="w-10 h-10 rounded-xl flex items-center justify-center transition-colors" title="Pilih (Cursor)">
          ...
          <button @click="clearCanvas" class="w-10 h-10 rounded-xl flex items-center justify-center text-slate-600 hover:bg-red-50 hover:text-red-600 transition-colors" title="Hapus Semua">
    ...
    <div class="w-full md:w-80 bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)] flex flex-col overflow-hidden">
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar flex flex-col" ref="chatContainer">
        ...
        <div v-for="(msg, idx) in chatMessages" :key="idx" class="flex flex-col" :class="msg.isSelf ? 'items-end' : 'items-start'">
          ...
          <div class="px-4 py-2 rounded-2xl max-w-[85%] text-sm" :class="msg.isSelf ? 'bg-orange-500 text-white rounded-br-sm' : 'bg-slate-100 text-slate-800 rounded-bl-sm'">
            {{ msg.text }}
          </div>
        </div>
      </div>
      ...
      <div class="p-4 border-t border-slate-100 bg-white">
        <form @submit.prevent="sendMessage" class="relative">
          <input v-model="newMessage" type="text" placeholder="Ketik pesan..." class="w-full bg-slate-50 border-none rounded-xl pl-4 pr-12 py-3 text-sm font-medium text-slate-800 outline-none focus:ring-2 focus:ring-orange-200 transition-all" />
          <button type="submit" :disabled="!newMessage.trim()" class="absolute right-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-orange-500 text-white rounded-lg flex items-center justify-center hover:bg-orange-600 disabled:opacity-50 transition-colors">
