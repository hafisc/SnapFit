  ...
  >
    ...
    <div class="h-20 flex items-center px-6 border-b border-borderSoft">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <span class="text-[10px] font-semibold text-muted block -mt-0.5 tracking-wide">Designer Studio</span>
        </div>
      </div>
    </div>
    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4 no-scrollbar">
      ...
      <router-link to="/designer/settings" @click="$emit('close')"
        class="flex items-center gap-3.5 px-3.5 py-3 rounded-xl mb-1 transition-all duration-200 text-[13px] font-semibold group relative"
        :class="[isActive('/designer/settings') ? 'bg-gradient-to-r from-orange-50 to-orange-50/50 text-terracotta shadow-sm' : 'text-muted hover:text-espresso hover:bg-slate-50']">
        <div v-if="isActive('/designer/settings')" class="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-terracotta rounded-r-full"></div>
        <div class="w-[28px] h-[28px] flex items-center justify-center transition-transform group-hover:scale-110"
          :class="[isActive('/designer/settings') ? 'text-terracotta' : 'text-slate-400 group-hover:text-muted']">
    ...
    <div class="p-4 border-t border-borderSoft bg-slate-50/50">
      <div class="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-surface transition-all cursor-pointer group border border-transparent hover:border-borderSoft hover:shadow-sm">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-[13px] font-bold text-espresso truncate">{{ user?.name || 'Designer' }}</p>
          <p class="text-[11px] text-muted truncate">{{ user?.email || 'designer@snapfit.id' }}</p>
        </div>
