<template>
  <div class="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
      <div>
        <h2 class="text-xl font-black text-slate-800 tracking-tight">Moderasi Ulasan</h2>
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-slate-50">
            ...
            <tr v-else v-for="review in reviews" :key="review.id" class="hover:bg-slate-50/50 transition-colors group">
              <td class="px-6 py-4">
                <span class="text-xs font-bold text-slate-800">{{ review.user.name }}</span>
              </td>
              <td class="px-6 py-4">
                <span class="text-xs font-bold text-slate-600 line-clamp-1">{{ review.product?.name || 'Produk Dihapus' }}</span>
              </td>
