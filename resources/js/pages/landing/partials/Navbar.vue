  ...
  <header
    ...
    <nav class="max-w-[1480px] mx-auto px-8 py-1.5 flex items-center relative">
      ...
      <div class="flex items-center gap-12">
        ...
        <a href="/marketplace" class="relative px-3 py-2 text-[13px] font-bold text-terracotta transition-colors flex flex-col items-center">
          Marketplace
          <span class="absolute -bottom-1 w-4 h-0.5 bg-terracotta rounded-full"></span>
        </a>
        ...
        <div class="relative notification-menu">
          ...
          <Transition enter-active-class="transition ease-out duration-200"
            ...
            <div v-if="showNotifications"
              class="absolute right-0 sm:-right-8 top-full mt-3 w-[320px] sm:w-[380px] bg-surface rounded-3xl shadow-2xl border border-borderSoft overflow-hidden z-50">
              <div class="bg-gray-50 px-5 py-4 border-b border-borderSoft flex justify-between items-center">
                <h3 class="font-black text-espresso text-sm">Notifikasi</h3>
                <button @click="markAllAsRead"
                  class="text-[11px] font-bold text-terracotta hover:text-orange-700 transition-colors">Tandai semua
                  dibaca</button>
              </div>
              <div class="max-h-[360px] overflow-y-auto hide-scrollbar">
                <div v-if="notifications.length === 0" class="p-8 text-center text-sm text-muted font-medium">
                  Belum ada notifikasi
                </div>
                <div v-else>
                  <div v-for="notif in notifications" :key="notif.id"
                    class="p-5 border-b border-gray-50 hover:bg-sand/40 transition-colors cursor-pointer flex gap-3 relative">
                    <div class="w-2.5 h-2.5 rounded-full mt-1 flex-shrink-0 transition-colors duration-300"
                      :class="notif.unread ? 'bg-terracotta shadow-sm shadow-terracotta/30' : 'bg-transparent'"></div>
                    <div>
                      <h4 class="text-sm font-black text-espresso">{{ notif.title }}</h4>
                      <p class="text-xs text-muted mt-1 leading-relaxed">{{ notif.message }}</p>
              ...
              <div
                class="p-4 border-t border-borderSoft text-center bg-gray-50/80 hover:bg-gray-100/80 transition-colors cursor-pointer">
                <button class="text-xs font-black text-espresso hover:text-terracotta transition-colors">Lihat Semua
                  Notifikasi</button>
              </div>
            </div>
          </Transition>
              ...
              <div v-if="showUserMenu"
                ...
                <div class="py-2">
                  ...
                  <template v-if="user.active_role === 'umkm'">
                    ...
                    <button class="w-full flex items-center gap-3 px-6 py-2.5 text-sm font-bold text-espresso hover:bg-sand hover:text-terracotta transition-all group">
