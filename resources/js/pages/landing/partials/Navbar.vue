<template>
  <header
    class="fixed top-0 left-0 right-0 z-50 bg-white/60 backdrop-blur-2xl border-b border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] animate-slide-down">
    ...
    <nav class="max-w-[1600px] mx-auto px-6 py-2.5 flex justify-between items-center relative">
      ...
      <a href="/" class="flex items-center gap-2.5 group cursor-pointer">
        <div
          class="w-10 h-10 rounded-xl overflow-hidden shadow-sm border border-slate-100 transform group-hover:scale-105 transition-all duration-300 flex items-center justify-center bg-white">
          <img :src="'/images/logo.png'" alt="SnapFit Logo" class="w-full h-full object-cover" />
        </div>
        <div>
          <h1
            class="font-black text-lg tracking-tight text-gray-900 leading-none group-hover:text-orange-600 transition-colors">
            SnapFit</h1>
      ...
      <div class="flex items-center gap-2 sm:gap-3">
        ...
        <div class="hidden md:flex relative items-center mr-2 search-container z-50">
          ...
          <svg class="w-4 h-4 text-gray-500 absolute left-3.5 pointer-events-none" fill="none" stroke="currentColor"
            viewBox="0 0 24 24">
          ...
          <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 translate-y-1"
            ...
            <div v-if="showSearchResults"
              class="absolute top-full mt-2 w-full bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-[60]">
              <div v-if="isSearching" class="p-4 text-center text-xs text-gray-500 font-bold">
                <svg class="animate-spin h-5 w-5 mx-auto text-orange-500 mb-2" xmlns="http://www.w3.org/2000/svg"
              ...
              <div v-else class="max-h-[60vh] overflow-y-auto hide-scrollbar">
                <a v-for="item in searchResults" :key="item.id" href="javascript:void(0)"
                  ...
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-[10px] text-orange-600 font-bold mt-0.5">Rp {{ Number(item.price ??
        ...
        <div class="relative notification-menu">
          <button @click="toggleNotifications"
            ...
            <svg class="w-4 h-4 text-gray-600 group-hover:text-orange-600 transition-colors" fill="none"
          ...
          <Transition enter-active-class="transition ease-out duration-200"
            ...
            <div v-if="showNotifications"
              class="absolute right-0 sm:-right-8 top-full mt-3 w-[320px] sm:w-[380px] bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden z-50">
              <div class="bg-gray-50 px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                ...
                <button @click="markAllAsRead"
                  class="text-[11px] font-bold text-orange-600 hover:text-orange-700 transition-colors">Tandai semua
                  dibaca</button>
              </div>
              <div class="max-h-[360px] overflow-y-auto hide-scrollbar">
                ...
                <div v-else>
                  <div v-for="notif in notifications" :key="notif.id"
                    class="p-5 border-b border-gray-50 hover:bg-orange-50/40 transition-colors cursor-pointer flex gap-3 relative">
                    <div class="w-2.5 h-2.5 rounded-full mt-1 flex-shrink-0 transition-colors duration-300"
                      :class="notif.unread ? 'bg-orange-500 shadow-sm shadow-orange-300' : 'bg-transparent'"></div>
                    <div>
              ...
              <div
                class="p-4 border-t border-gray-100 text-center bg-gray-50/80 hover:bg-gray-100/80 transition-colors cursor-pointer">
                <button class="text-xs font-black text-gray-700 hover:text-orange-600 transition-colors">Lihat Semua
                  Notifikasi</button>
              </div>
            </div>
          </Transition>
        ...
        <div class="relative cart-menu">
          <button @click="toggleCart" class="relative p-1.5 rounded-lg hover:bg-gray-100 transition-colors group">
            ...
            <span v-if="cartCount > 0"
              class="absolute -top-1 -right-1 w-5 h-5 bg-orange-500 text-white text-[10px] font-black rounded-full flex items-center justify-center border border-white">
          ...
          <Transition enter-active-class="transition ease-out duration-200"
            ...
            leave-to-class="opacity-0 scale-95 -translate-y-2">
            <div v-if="showCart"
              class="absolute right-0 sm:-right-8 top-full mt-3 w-[320px] sm:w-[380px] bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden z-50 flex flex-col">
              <div class="bg-gray-50 px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                <h3 class="font-black text-gray-900 text-sm">Keranjang Belanja <span
                    class="text-gray-500 font-medium">({{ cartCount }})</span></h3>
              </div>
              <div class="max-h-[360px] overflow-y-auto hide-scrollbar">
                ...
                <div v-else>
                  <div v-for="item in cartStore.items.slice(0, 4)" :key="`${item.id}-${item.variant}`"
                    ...
                    <div class="flex-1 min-w-0 flex flex-col justify-center">
                      ...
                      <p class="text-xs font-black text-orange-600 mt-0.5">{{ cartStore.formatCurrency(item.price) }}
                      </p>
                      <div class="flex items-center gap-1.5 mt-1.5">
                        <button @click.stop="decrementItem(item)"
                          class="w-6 h-6 rounded-lg bg-gray-100 hover:bg-orange-100 text-gray-600 hover:text-orange-600 flex items-center justify-center text-xs font-black transition-colors">−</button>
                  ...
                  <div v-if="cartStore.items.length > 4"
                    class="p-3 text-center text-xs font-bold text-gray-500 bg-gray-50">
              ...
              <div v-if="cartStore.items.length > 0" class="p-4 border-t border-gray-100 bg-white">
                ...
                <button @click="goToCart"
                  class="w-full bg-orange-600 text-white py-2.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-orange-700 hover:shadow-lg transition-all active:scale-[0.98]">
                  Lihat Keranjang
                </button>
              </div>
            </div>
          ...
          <div class="relative user-menu">
            ...
            <Transition enter-active-class="transition ease-out duration-200"
              ...
              <div v-if="showUserMenu"
                class="absolute right-0 top-full mt-3 w-64 bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden z-50">
                <!-- User Header -->
                <div class="bg-orange-50 px-6 py-5 border-b border-orange-100">
                  <div class="flex items-center gap-3">
                    <div
                      class="w-14 h-14 rounded-2xl overflow-hidden bg-white flex items-center justify-center shadow-lg border border-orange-200">
                      <img
                        :src="user.avatar || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.name}&scale=130&backgroundColor=ffdfbf,c0aede,d1d4f9`"
                        alt="Avatar" class="w-full h-full object-cover" />
                    </div>
                ...
                <div class="py-2">
                  <button @click="goToProfile"
                    class="w-full flex items-center gap-3 px-6 py-3 text-sm font-bold text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-all group">
                    <div
                      class="w-8 h-8 rounded-xl bg-gray-100 group-hover:bg-orange-100 flex items-center justify-center transition-colors">
                  ...
                  <button @click="goToCart"
                    ...
                    <span>Cart</span>
                    <span v-if="cartCount > 0"
                      class="ml-auto bg-orange-500 text-white text-xs font-black px-2 py-1 rounded-full">
                      {{ cartCount }}
                    </span>
                  </button>
        ...
        <button v-else @click="goToLogin"
          class="group relative bg-gray-900 text-white px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-orange-600 transition-all duration-300 shadow-md hover:shadow-lg hover:shadow-orange-200/50 hover:-translate-y-0.5 ml-1">
    ...
    <Transition enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 -translate-y-4"
      ...
      <div v-if="showMobileMenu" class="lg:hidden border-t border-gray-100 bg-white/95 backdrop-blur-xl">
        <div class="max-w-[1600px] mx-auto px-6 py-4 space-y-2">
          <a href="#marketplace"
            class="block px-4 py-3 rounded-xl text-sm font-bold text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-all">Marketplace</a>
          <a href="#features"
            class="block px-4 py-3 rounded-xl text-sm font-bold text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-all">AI
            Studio</a>
          <a href="#features"
            class="block px-4 py-3 rounded-xl text-sm font-bold text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-all">Co-Create</a>
          <a href="#features"
            class="block px-4 py-3 rounded-xl text-sm font-bold text-gray-700 hover:bg-orange-50 hover:text-orange-600 transition-all">AR
            Try-On</a>
        </div>
      </div>
    </Transition>
