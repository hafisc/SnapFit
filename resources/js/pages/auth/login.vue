        ...
        <div class="max-w-md mx-auto w-full">
          ...
          <div class="mb-8 md:mb-10 text-center md:text-left">
            <h3 class="text-2xl font-black text-[#2B1E16] mb-2 tracking-tight">
              {{ isForgotPassword ? 'Lupa Kata Sandi?' : (isLogin ? 'Selamat Datang Kembali' : 'Mulai Perjalanan Kreatif Anda') }}
            </h3>
            <p class="text-[#6F6259] text-[13px] font-medium leading-relaxed">
              {{ isForgotPassword
                ...
                : (isLogin ? 'Masuk untuk mengelola pesanan, toko, studio AI, dan kolaborasi kreatif Anda.' : 'Daftar sebagai pembeli terlebih dahulu, lalu buka toko atau bergabung sebagai designer setelah login.') }}
            </p>
          </div>
          ...
          <form class="space-y-4" @submit.prevent="submit">
            ...
            <div v-if="isForgotPassword" class="space-y-4">
              ...
              <div class="pt-4 text-center">
                <p class="text-[11px] text-slate-400 font-bold uppercase tracking-widest">
                  <button type="button" @click="isForgotPassword = false; errorMessage = ''; successMessage = ''"
                    class="text-[#2B1E16] font-black hover:text-[#B85C38] transition-colors border-b-2 border-[#B85C38]/20 hover:border-[#B85C38] pb-0.5">
                    Kembali ke Login
                  </button>
                </p>
              </div>
            ...
            <div v-else class="space-y-4">
              ...
              <Transition name="slide-fade">
                <div v-if="!isLogin">
                  <label class="block text-[10px] font-bold uppercase tracking-wider text-[#6F6259] mb-1.5 ml-1">Nama Lengkap</label>
                  <div class="relative group">
                    <span
                      class="absolute left-4 top-1/2 -translate-y-1/2 text-[#E8DCCB] group-focus-within:text-[#B85C38] transition-colors pointer-events-none">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <input v-model="name" type="text" placeholder="Nama Lengkap" :required="!isLogin"
                      class="w-full bg-[#FFFCF7] border border-[#E8DCCB] rounded-xl pl-11 pr-5 py-3 text-sm outline-none focus:ring-[3px] focus:ring-[#B85C38]/10 focus:border-[#B85C38] hover:border-[#B85C38]/50 transition-all text-[#2B1E16] font-medium placeholder-[#6F6259]/50 shadow-sm" />
                  </div>
                </div>
              </Transition>
              ...
              <!-- Password -->
              <div>
                <label class="block text-[10px] font-bold uppercase tracking-wider text-[#6F6259] mb-1.5 ml-1">Kata Sandi</label>
                <div class="relative group">
                  <span
                    class="absolute left-4 top-1/2 -translate-y-1/2 text-[#E8DCCB] group-focus-within:text-[#B85C38] transition-colors pointer-events-none">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="flex justify-end mt-2">
                  <button v-if="isLogin" type="button" @click="isForgotPassword = true; errorMessage = ''; successMessage = ''"
              ...
              <div class="relative flex items-center py-2">
                <div class="flex-grow border-t border-borderSoft"></div>
                <span class="flex-shrink-0 mx-4 text-[#6F6259]/50 text-[10px] font-bold uppercase tracking-widest">Atau</span>
                <div class="flex-grow border-t border-borderSoft"></div>
              </div>
              ...
              <div class="pt-4 text-center">
                <p class="text-[11px] text-[#6F6259]/60 font-bold uppercase tracking-widest">
  ...
  try {
    ...
    if (!res.ok) {
      if (data.errors) {
        ...
        errorMessage.value = data.message || 'Gagal mengirim email reset sandi.';
      }
      return;
    }
