  ...
  <div class="min-h-screen bg-gradient-to-b from-slate-50 to-white font-sans">
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 py-12 pt-32">
      ...
      <div class="text-center mb-12">
        ...
        <h1 class="text-4xl font-black text-slate-900 mb-3">Daftar sebagai UMKM</h1>
      ...
      <div class="bg-white rounded-3xl border border-slate-100 shadow-lg p-8 sm:p-10 mb-8">
        <form @submit.prevent="handleSubmit" class="space-y-6">
          <!-- Business Name -->
          <div>
            <label for="business_name" class="block text-sm font-bold text-slate-900 mb-2">
            ...
            <input
              ...
              class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-orange-500 focus:outline-none transition-colors text-slate-900 font-medium placeholder:text-slate-400"
              required
              :disabled="isLoading"
            />
            <p class="text-xs text-slate-500 mt-2">Nama bisnis Anda yang akan ditampilkan di marketplace</p>
            <p v-if="errors.business_name" class="text-xs text-red-500 mt-1">{{ errors.business_name }}</p>
          </div>

          <!-- Business Phone -->
          <div>
            <label for="business_phone" class="block text-sm font-bold text-slate-900 mb-2">
            ...
            <input
              ...
              :disabled="isLoading"
            />
            <p class="text-xs text-slate-500 mt-2">Nomor yang bisa dihubungi customer Anda</p>
            <p v-if="errors.business_phone" class="text-xs text-red-500 mt-1">{{ errors.business_phone }}</p>
          </div>

          <!-- Business Address -->
          <div>
            <label for="business_address" class="block text-sm font-bold text-slate-900 mb-2">
              Alamat Bisnis
            </label>
            <textarea
              id="business_address"
              ...
              class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-orange-500 focus:outline-none transition-colors text-slate-900 font-medium placeholder:text-slate-400 resize-none"
              :disabled="isLoading"
            ></textarea>
            <p class="text-xs text-slate-500 mt-2">Alamat lengkap lokasi bisnis Anda</p>
            <p v-if="errors.business_address" class="text-xs text-red-500 mt-1">{{ errors.business_address }}</p>
          </div>

          <!-- Business Bio -->
          <div>
            <label for="business_bio" class="block text-sm font-bold text-slate-900 mb-2">
          ...
          <div class="pt-6">
            <button
              type="submit"
              :disabled="isLoading"
              class="w-full bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-700 hover:to-amber-700 disabled:opacity-50 text-white font-black py-4 px-6 rounded-lg transition-all duration-200 active:scale-95 text-lg uppercase tracking-widest shadow-lg shadow-orange-500/30 flex items-center justify-center gap-2"
            >
      ...
      <div class="text-center mt-8">
        <router-link to="/profile" class="inline-flex items-center gap-2 text-orange-600 hover:text-orange-700 font-semibold transition-colors">
