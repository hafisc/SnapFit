  ...
  <div class="min-h-screen bg-gradient-to-b from-slate-50 to-white font-sans">
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 py-12 pt-32">
      ...
      <div class="text-center mb-12">
        ...
        <h1 class="text-4xl font-black text-slate-900 mb-3">Daftar sebagai Designer</h1>
      ...
      <!-- Form Card -->
      <div class="bg-white rounded-3xl border border-slate-100 shadow-lg p-8 sm:p-10 mb-8">
        <form @submit.prevent="handleSubmit" class="space-y-6">
          <!-- Portfolio URL -->
          <div>
            <label for="designer_portfolio_url" class="block text-sm font-bold text-slate-900 mb-2">
              URL Portfolio / Website
            </label>
            <input
              id="designer_portfolio_url"
              v-model="form.designer_portfolio_url"
              type="url"
              placeholder="Contoh: https://portfolio.yourname.com atau https://dribbble.com/yourname"
              class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-purple-500 focus:outline-none transition-colors text-slate-900 font-medium placeholder:text-slate-400"
              :disabled="isLoading"
            />
            <p class="text-xs text-slate-500 mt-2">Link ke portfolio online Anda yang menampilkan karya terbaik (Dribbble, Behance, personal website, dll)</p>
            <p v-if="errors.designer_portfolio_url" class="text-xs text-red-500 mt-1">{{ errors.designer_portfolio_url }}</p>
          </div>

          <!-- Designer Name -->
          <div>
            <label for="designer_name" class="block text-sm font-bold text-slate-900 mb-2">
            ...
            <input
              ...
              :disabled="isLoading"
            />
            <p class="text-xs text-slate-500 mt-2">Nama yang akan ditampilkan di profil designer Anda</p>
            <p v-if="errors.designer_name" class="text-xs text-red-500 mt-1">{{ errors.designer_name }}</p>
          </div>

          <!-- Designer Bio -->
          <div>
            <label for="designer_bio" class="block text-sm font-bold text-slate-900 mb-2">
              Bio / Deskripsi Diri
            </label>
            <textarea
              id="designer_bio"
              v-model="form.designer_bio"
              rows="4"
              ...
              class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-purple-500 focus:outline-none transition-colors text-slate-900 font-medium placeholder:text-slate-400 resize-none"
              maxlength="500"
              :disabled="isLoading"
            ></textarea>
          ...
          <!-- Specialization -->
          <div>
            <label for="specialization" class="block text-sm font-bold text-slate-900 mb-2">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <label v-for="spec in specializations" :key="spec" class="flex items-center gap-2 cursor-pointer group">
                <input
                  ...
                  :disabled="isLoading"
                />
                <span class="text-sm text-slate-700 group-hover:text-purple-600 transition-colors">{{ spec }}</span>
              </label>
            </div>
            <p class="text-xs text-slate-500 mt-2">Pilih minimal 1 spesialisasi yang Anda kuasai</p>
            <p v-if="errors.specializations" class="text-xs text-red-500 mt-1">{{ errors.specializations }}</p>
          </div>

          <!-- Contact Info -->
          <div>
            <label for="contact_email" class="block text-sm font-bold text-slate-900 mb-2">
            ...
            <input
              ...
              type="email"
              placeholder="Contoh: designer@email.com"
              class="w-full px-4 py-3 border-2 border-slate-200 rounded-lg focus:border-purple-500 focus:outline-none transition-colors text-slate-900 font-medium placeholder:text-slate-400"
              :disabled="isLoading"
            />
            <p class="text-xs text-slate-500 mt-2">Email kontak opsional jika berbeda dengan email account SnapFit Anda</p>
        ...
        <!-- Success Message -->
        <div v-if="successMessage" class="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-start gap-3">
      ...
      <div class="text-center mt-8">
        <router-link to="/profile" class="inline-flex items-center gap-2 text-purple-600 hover:text-purple-700 font-semibold transition-colors">
